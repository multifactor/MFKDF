...
suite('factors/hmacsha1', () => {
  ...
  suite('errors', async () => {
    test('id/type', async () => {
      mfkdf.setup.factors.hmacsha1({ id: 12345 }).should.be.rejectedWith(TypeError)
    })

    test('id/range', async () => {
      mfkdf.setup.factors.hmacsha1({ id: '' }).should.be.rejectedWith(RangeError)
    })

    test('secret/type', async () => {
      mfkdf.setup.factors.hmacsha1({ secret: 12345 }).should.be.rejectedWith(TypeError)
    })

    test('secret/range', async () => {
      mfkdf.setup.factors.hmacsha1({ secret: Buffer.from('12345') }).should.be.rejectedWith(RangeError)
    })
